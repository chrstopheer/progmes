...
export default function Login() {
  ...
  const handleLogin = async () => {
    setBusy(true);
    try { await signInWithGoogle(); toast.success("Login realizado com sucesso."); navigate("/", { replace: true }); }
    catch (error) {
      const message = error?.code === "auth/popup-closed-by-user" ? "A janela de login foi fechada." : error?.code === "auth/unauthorized-domain" ? "Este domínio do Preview ainda não foi autorizado no Firebase." : error?.code === "auth/popup-blocked" ? "O navegador bloqueou a janela do Google. Tente novamente ou permita popups para este site." : "Não foi possível entrar com o Google. Tente novamente.";
      setErrorDetail(`${error?.code || "firebase/error"}: ${error?.message || message}`); toast.error(message);
    } finally { setBusy(false); }
  };

  const handleLocalAccess = () => { enterLocalAccess(); navigate("/", { replace: true }); };

  return (
    <main className="min-h-screen bg-paper flex items-center justify-center px-5 py-4 sm:p-6">
      <section className="w-full max-w-sm rounded-[2rem] bg-white border shadow-sm px-7 py-6 sm:px-9 sm:py-8 text-center" style={{ borderColor: "var(--hairline)" }}>
        <div className="flex flex-col items-center"><div className="h-20 w-20 rounded-3xl flex items-center justify-center shadow-sm mb-5" style={{ background: "var(--brand-yellow)" }}><CalendarDays className="h-10 w-10" style={{ color: "var(--brand-red)" }} /></div><h1 className="font-display text-4xl leading-tight" style={{ color: "var(--ink)" }}>Programação</h1><p className="mt-2 text-base" style={{ color: "var(--ink-soft)" }}>Crie e compartilhe.</p></div>
        {!configured && <div className="rounded-xl border p-4 mt-5 text-sm leading-6 text-left" style={{ borderColor: "#f0c36d", background: "#fff8e7", color: "#7a4e00" }}>O Firebase ainda não foi configurado. Adicione as variáveis `REACT_APP_FIREBASE_*` antes de iniciar o app.</div>}
