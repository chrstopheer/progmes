...
export default function Login() {
  ...
  const handleLocalAccess = () => navigate("/", { replace: true });
    ...
    <main className="min-h-screen bg-paper flex items-center justify-center px-5 py-4 sm:p-6">
      ...
      >
        <div className="flex flex-col items-center">
          <div className="h-20 w-20 rounded-3xl flex items-center justify-center shadow-sm mb-5" style={{ background: "var(--brand-yellow)" }}>
            <CalendarDays className="h-10 w-10" style={{ color: "var(--brand-red)" }} />
          </div>
          <h1 className="font-display text-4xl leading-tight" style={{ color: "var(--ink)" }}>Programação</h1>
          <p className="mt-2 text-base" style={{ color: "var(--ink-soft)" }}>Crie e compartilhe.</p>
        </div>

        {!configured && (
          <div className="rounded-xl border p-4 mt-5 text-sm leading-6 text-left" style={{ borderColor: "#f0c36d", background: "#fff8e7", color: "#7a4e00" }}>
            O Firebase ainda não foi configurado. Adicione as variáveis `REACT_APP_FIREBASE_*` antes de iniciar o app.
          </div>
        )}

        {errorDetail && (
          <div className="rounded-xl border p-4 mt-5 text-xs leading-5 break-words text-left" style={{ borderColor: "#e2a3a3", background: "#fff1f1", color: "#8a1c1c" }} data-testid="firebase-error-detail">
            <strong>Detalhe técnico:</strong><br />{errorDetail}
          </div>
        )}

        <Button className="w-full h-12 mt-5 rounded-md border bg-white hover:bg-[#f8fafd] text-[#1f1f1f] font-medium text-sm" onClick={handleLogin} disabled={!configured || busy} data-testid="login-submit-button" aria-label="Entrar com o Google" style={{ borderColor: "#747775", fontFamily: "Google Sans, Arial, sans-serif" }}>
          {busy ? <Loader2 className="h-5 w-5 mr-3 animate-spin" /> : (
        ...
        <div className="mt-4 pt-3 border-t" style={{ borderColor: "var(--hairline)" }}>
          <button type="button" onClick={handleLocalAccess} className="text-sm font-medium underline-offset-4 hover:underline" style={{ color: "var(--brand-blue)" }} data-testid="login-without-google-btn">
