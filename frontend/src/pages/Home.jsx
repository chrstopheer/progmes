    ...
    <div className="min-h-screen bg-paper">
      ...
      <main className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
            ...
            <div className="flex gap-3 flex-wrap">
              <div className="flex flex-col">
                ...
                <Select value={String(month)} onValueChange={(value) => setMonth(parseInt(value, 10))}>
                  ...
                  <SelectContent>{MONTHS_PT.map((m, i) => <SelectItem key={m} value={String(i)} data-testid={`month-option-${i}`}>{m}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="flex flex-col">
                ...
                <Select value={String(year)} onValueChange={(value) => setYear(parseInt(value, 10))}>
                  ...
                  <SelectContent>{yearsRange(6).map((y) => <SelectItem key={y} value={String(y)} data-testid={`year-option-${y}`}>{y}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            </div>
          </div>
        ...
        <section className="rounded-2xl bg-white border shadow-sm p-4 sm:p-6" style={{ borderColor: "var(--hairline)" }} data-testid="calendar-grid-section">
          ...
          <div className="grid grid-cols-7 gap-1 sm:gap-2" data-testid="calendar-grid">
            {cells.map((d, idx) => {
              ...
              const activityLabel = Array.isArray(dayActivities) && dayActivities.length > 1 ? `${dayActivities.length} atividades` : dayActivities?.[0]?.division || "•";
              return (
                <button key={idx} onClick={() => navigate(`/atividade/${year}/${month}/${d}`)} data-testid={`day-${d}`} className="calendar-cell-btn aspect-square rounded-xl border flex flex-col items-center justify-center p-1 relative" style={{ borderColor: today ? "var(--brand-red)" : "var(--hairline)", background: filled ? "var(--brand-blue-soft)" : "white", borderWidth: today ? 2 : 1 }}>
        ...
        <section className="flex flex-col sm:flex-row gap-3 sm:justify-between">
          <Button onClick={() => navigate(`/programacao/${year}/${month}`)} data-testid="view-schedule-btn" className="flex-1" style={{ background: "var(--brand-red)", color: "white" }}><FileText className="h-4 w-4 mr-2" /> Ver programação de {MONTHS_PT[month]}</Button>
        </section>
        ...
        {savedMonths.length > 0 && (
          <section className="rounded-2xl bg-white border shadow-sm p-5" style={{ borderColor: "var(--hairline)" }} data-testid="saved-months-section">
            <div className="flex items-center gap-2 mb-3"><Archive className="h-4 w-4" style={{ color: "var(--brand-blue)" }} /><h2 className="font-display text-lg">Programações salvas</h2></div>
            <div className="flex flex-wrap gap-2">
              {savedMonths.map((sm) => (
                <button
                  key={sm.key}
                  onClick={(event) => handleSavedProgrammingClick(event, sm)}
