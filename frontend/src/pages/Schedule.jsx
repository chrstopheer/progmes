...
export default function Schedule() {
  ...
  useEffect(() => {
    const updateSize = () => {
      ...
      const scale = Math.min(1, (container.clientWidth - 24) / 720); setPreviewScale(scale);
      if (previewRef.current) { const el = previewRef.current; const prevTransform = el.style.transform; el.style.transform = "none"; const naturalH = el.getBoundingClientRect().height; el.style.transform = prevTransform; setPreviewHeight(Math.ceil(naturalH * scale) + 16); }
    };
    updateSize(); const t = setTimeout(updateSize, 100); window.addEventListener("resize", updateSize); return () => { clearTimeout(t); window.removeEventListener("resize", updateSize); };
  }, [monthData, settings]);
  ...
  const handleDownload = () => downloadMonthPdf({ year: y, month: m, settings, monthData });
  const shareTitle = `Programação do Mês de ${MONTHS_PT[m]} de ${y}`;
  const handleShareWhatsApp = async () => {
    try {
      const file = buildMonthPdfFile({ year: y, month: m, settings, monthData });
      const shareData = { files: [file], title: shareTitle, text: `${shareTitle}${settings.header.community ? `\n${settings.header.community}` : ""}` };
      if (navigator.canShare && navigator.canShare({ files: [file] }) && navigator.share) { await navigator.share(shareData); return; }
      downloadMonthPdf({ year: y, month: m, settings, monthData });
      const text = encodeURIComponent(`📅 *${shareTitle}*${settings.header.community ? `\n${settings.header.community}` : ""}\n\n(Anexe o PDF baixado.)`);
