...
function settingsWithCommunityDefault(settings) { if (settings?.header?.community) return settings; return { ...settings, header: { ...settings.header, community: COMMUNITY_DEFAULT } }; }
    ...
    <Dialog open={open} onOpenChange={handleDialogOpenChange}>
      <DialogContent data-testid="settings-dialog" className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-5 py-2">
          ...
          <section className="space-y-3"><h3 className="text-sm font-semibold uppercase tracking-wider" style={{ color: "var(--brand-blue)" }}>Rodapé</h3><Textarea id="footer" data-testid="setting-footer" rows={2} value={settings.footer} onChange={(e) => setSettings((s) => ({ ...s, footer: e.target.value }))} {...textInputProps} className="not-italic" /></section>
        </div>
        <DialogFooter className="gap-2 sm:justify-between"><Button type="button" variant="ghost" data-testid="settings-reset-btn" onClick={handleReset} style={{ background: "var(--brand-yellow)", color: "var(--ink)" }}>Restaurar padrão</Button><div className="flex gap-2"><Button type="button" variant="outline" data-testid="settings-cancel-btn" onClick={closeDialog}>Cancelar</Button><Button type="button" data-testid="settings-save-btn" onClick={handleSave} style={{ background: "var(--brand-red)", color: "white" }}>Salvar</Button></div></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
