...
export const SchedulePreview = React.forwardRef(function SchedulePreview(
  ...
  const community = settings?.header?.community || COMMUNITY_DEFAULT;
  return (
    <div ref={ref} data-testid="schedule-preview" style={{ width: 720, padding: 16, background: "#fff", transform: `scale(${scale})`, transformOrigin: "top left" }}>
      <div className="preview-header"><div className="h-title">{title}</div><div className="h-sub">{community}</div><div className="h-quote">{settings?.header?.quote || ""}</div></div>
      <table className="preview-table">
        <colgroup><col style={{ width: COL_DATA }} /><col style={{ width: COL_DIA }} /><col style={{ width: COL_DIVISAO }} /><col /><col style={{ width: COL_LOCAL }} /><col style={{ width: COL_HORARIO }} /></colgroup>
